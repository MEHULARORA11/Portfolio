...
const PortfolioLoader = ({
  ...
  useEffect(() => {
    const doneByProgress =
      !isControlled && progress >= 100;

    const doneByFlag =
      isControlled && isLoading === false;

    if (
      (doneByProgress || doneByFlag) &&
      phase === "active"
    ) {
      const t = setTimeout(() => {
        setPhase("morphing");
  ...
  useLayoutEffect(() => {
    ...
    if (cardEl && targetEl) {
      ...
      cardEl.style.transition =
        `transform ${flightMs}ms cubic-bezier(0.65,0,0.35,1)`;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            `radial-gradient(circle at 50% 38%, ${fb.bg2}, ${fb.bg1} 72%)`,
        }}
      >
        {particles.map((p) => (
          <span
        ...
        >
          <div
            ...
            style={{
              ...
              boxShadow:
                `0 30px 60px -20px rgba(0,0,0,0.5), 0 0 50px ${fb.glow}`,
            }}
          >
            {/* top bar */}
            <div
